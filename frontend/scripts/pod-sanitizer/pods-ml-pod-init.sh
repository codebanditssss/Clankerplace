...
# configured yet.  Save credentials via the pods.ml Connectors tab to make
...
(
  ...
  # touch-file lets the user (or pods.ml) explicitly pause the supervisor:
...
# via the pods.ml console (which is `docker exec -it bash`), not stdin to
# this PID 1.
exec tail -f /dev/null
